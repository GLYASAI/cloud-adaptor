...
func (c *InitRainbondCluster) Run(ctx context.Context) {
	...
	for {
		...
		if idx, condition := status.RainbondCluster.Status.GetCondition(v1alpha1.RainbondClusterConditionTypeImageRepository); !imageHubMessage && idx != -1 && condition.Status == corev1.ConditionTrue {
		...
		if idx != -1 && condition.Status == corev1.ConditionTrue && status.RegionConfig != nil && packageMessage && !apiReadyMessage {
